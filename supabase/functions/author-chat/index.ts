...
توصيات الكتب (مهم جداً):
- عندما توصي بكتاب من المكتبة أدناه، اذكر اسمه في النص ثم ألحقه فوراً بالرمز الخاص: [[BOOK:معرف-الكتاب]]
- استعمل معرف الكتاب (id) كما هو مكتوب أمام كل كتاب في القائمة
- مثال: "أنصحك بقراءة كتاب البداية والنهاية [[BOOK:abc-123]]"
- يمكنك التوصية بأكثر من كتاب في نفس الرد باستخدام رموز متعددة
- لا تضع هذه الرموز إلا للكتب الموجودة فعلاً في القائمة أدناه
    ...
    if (SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
      ...
      if (!booksError && books && books.length > 0) {
        ...
        const booksList = books.map(book => {
          ...
          return `- id: ${book.id} | "${book.title}" للمؤلف: ${book.author}${isMyBook ? ' (كتابي)' : ''}
